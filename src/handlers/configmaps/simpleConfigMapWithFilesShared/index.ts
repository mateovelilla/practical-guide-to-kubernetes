...
export class SimpleConfigmapWithFilesSharedHandler extends AbstractHandler {
    public async handle(request: Request) {
        try {
            const loader = spinner()
            const statusPodsChecker = new StatusPodsCheckerHandler()
            loader.start(`Creating Creating Config!`);
            const { stdout: outputConfigmap, stderr: outputConfigmapError } =  await super.runCommand("kubectl",["create", "cm", "my-config",`--from-file=${__dirname}/prometheus-conf.yml`])
